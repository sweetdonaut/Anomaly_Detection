"""
Bottleneck Feature Analysis for Standard Compact Model
=====================================================

Analyze and visualize features extracted from Standard Compact Autoencoder (without residual connection) bottleneck layer
"""
...
class StandardCompactAutoencoder(nn.Module):
    ...
    def get_latent_features(self, x):
        """Extract latent representation (after expansion, ready for decoder)"""
        encoded = self.encoder(x)
        bottleneck_output = self.bottleneck(encoded)
...
def visualize_bottleneck_features(model, image_tensor, image_name, save_path, device='cuda'):
    ...
    with torch.no_grad():
        # Get bottleneck features (using expanded features for consistency)
...
def load_model(model_path, device='cuda'):
    ...
    model = StandardCompactAutoencoder(input_size=(176, 976), latent_dim=256)
...
def main():
    ...
    # Load model
    print("Loading model...")
    model = load_model('models/standard_compact_model.pth', device)
    ...
    with open(stats_path, 'w') as f:
        ...
        f.write(f"Model: Standard Compact Autoencoder (without residual connection)\n\n")
